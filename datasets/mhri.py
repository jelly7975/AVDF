...
class MHRI (data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        name = os.path.basename(self.data[index]['video_path']).split('.')[0]
        name = name.split('_')[0]
        ...
        if self.data_type == 'video' or self.data_type == 'audiovisual':        
            ...
            clip = self.load(path)
            clip = torch.from_numpy(clip).permute(1, 0, 2, 3)
            clip = torch.stack(tuple(clip), 0)
            
            if self.data_type == 'video':
                return clip, target, name
        ...
        if self.data_type == 'audio' or self.data_type == 'audiovisual':
            ...
            if self.data_type == 'audio':
                return audio_features, target, name

        if self.data_type == 'audiovisual':
            return audio_features, clip, target, name
